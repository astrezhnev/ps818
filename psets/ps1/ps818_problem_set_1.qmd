...
> **Assigned:** Wednesday, September 9, 2026. **Due:** Wednesday, September 23, 2026.
>
> Submit your solutions on **Gradescope**. Upload *both* the rendered `.html` file (saved as `Yourlastname_Yourfirstinitial_pset1.html`) and the `.qmd` source file that produced it (saved as `Yourlastname_Yourfirstinitial_pset1.qmd`). We should be able to run your code without error messages. In order to receive credit, homework submissions must be substantially started and all work must be shown.
>
...
The codebook for the dataset is included in the `data/` sub-folder as `ucdp-brd-codebook-221.pdf`.
...
Hint: Numerical optimization is messy here - two suggestions:

First, pay attention to your **starting values**. A vector of zeros implies a predicted count of $\text{exp}(0) = 1$ for every observation. You can see this is quite far from the observed data. Starting the intercept at something sensible like the (transformed) sample average works well here.

Second, **rescale `year`**. Using `year - 1989` in place of `year` puts the covariate on a $0$ to $32$ scale. This is useful because `maxLik` differentiates your log-likelihood *numerically* and can run into issues with numerical overflow/underflow with extreme inputs. 
...
Hint: You may find the `estfun()` function from `maxLik` (and the `sandwich` library) useful for this part. 
...
Using the UCDP data, use your function from Part A and the `maxLik` R package to obtain the MLE for the coefficients of a negative binomial regression that regresses the total number of battle deaths in a given conflict-year (using the "best" estimate: `bd_best`) on the year of observation, an indicator for whether the conflict is "interstate", and an indicator for whether the conflict is "internationalized intrastate" (the "left out" group for these dummies is 'intrastate').
